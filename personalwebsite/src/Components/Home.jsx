...
function HomePage() {
  ...
  const navItems = [
    ...
    { icon: Mail, label: "Email" },
    {
      icon: Instagram,
      label: "Instagram",
      href: "https://www.instagram.com/rayyatttz",
      external: true,
    },
    {
      icon: Github,
          ...
          {navItems.map((item, index) => {
            ...
            return (
              ...
              >
                {/* Tooltip */}
                <div
                  className={`tooltip ${isHovered ? "tooltip-visible" : ""}`}
                >
                  {item.label}
                ...
                {/* Icon Button */}
                <button
                  className={`icon-button ${isHovered ? "icon-button-hovered" : ""}`}
                  onClick={() => {
                    handleClick(item);
                  }}
                >
                  {/* Glow effect */}
                  <div
                    className={`glow-effect ${isHovered ? "glow-visible" : ""}`}
                  ></div>
